									...
									foreach($_fArray AS $_track => $_sArray){
										...
										echo '<ol class="carousel-indicators">';
											...
											foreach($_race_time AS $_k => $_v){
												...
												echo ($_v->race_uid == $_raceid) ? '<li data-target="/form/'.$_group.'/'.$_v->race_uid.'/'.$_v->track_date.'" data-slide-to="0" class="active"></li>' : '<a href="/form/'.$_group.'/'.$_v->race_uid.'/'.$_v->track_date.'" data-type="page-transition"><li data-target="/form/'.$_group.'/'.$_v->race_uid.'/'.$_v->track_date.'" data-slide-to="0"></li></a>';
											...
											foreach($_race_time AS $_k => $_v){
												...
												if($_v->race_uid == $_raceid){
													...
													if(get_next_raceid($_sArray->race_details->track_uid, $_v->race_time, $_v->track_date, 'less', $_group) == '#'){
														...
														<a class="left carousel-control" href="'. $folder .'/form/' . $_group . '/'.get_next_raceid($_sArray->race_details->track_uid, $_v->race_time, $_v->track_date, 'less', $_group).'/'.$_v->track_date.'" data-slide="prev" data-type="page-transition">
													...
													if(get_next_raceid($_sArray->race_details->track_uid, $_v->race_time, $_v->track_date, 'greater', $_group) == '#'){
														...
														<a class="right carousel-control" href="'. $folder .'/form/' . $_group . '/'.get_next_raceid($_sArray->race_details->track_uid, $_v->race_time, $_v->track_date, 'greater', $_group).'/'.$_v->track_date.'" data-slide="next" data-type="page-transition">
									...
									foreach($_races AS $_date => $_fArray){
										...
										foreach($_fArray AS $_track => $_sArray){
											...
											foreach($_sArray->runners AS $_runnerId=> $_tArray){
												...
												echo '  <a href="'. $folder .'/dogs/' . preg_replace('/\s+/', '', $_track) . '/' . $_raceid . '/' . $_date . '/' . $_runnerId . '" data-type="page-transition">
